...
def extra_layer(vgg, cfg):
    feat_layers,mlm_1,mlm_2, concat_layers, concat_layers_2, scale = [],[], [],[],[], 1

    for k, v in enumerate(cfg):
        #print("k:", k)
        if k%2==1:
            feat_layers += [FeatLayer(v[0], v[1], v[2])]
            mlm_1+= [FeatLayer(v[0], v[1], v[2])]
            mlm_2+= [FeatLayer(v[0], v[1], v[2])]
        else:
            feat_layers +=[FeatLayer_ed(v[0],v[1],v[2])]
            mlm_1 += [FeatLayer_ed(v[0], v[1], v[2])]
            mlm_2 += [FeatLayer_ed(v[0], v[1], v[2])]
    ...
    return vgg, feat_layers,mlm_1,mlm_2
...
# DSS network
class DSS(nn.Module):
    def __init__(self, base, feat_layers,mlm_1,mlm_2,e_feat_layers):
        ...
        self.feat_mlm_1=nn.ModuleList(mlm_1)
        self.feat_mlm_2=nn.ModuleList(mlm_2)
    ...
    def forward(self, x, xe):
        edges,xx1,xx,m,m_mlm1,m_mlm2,e, prob, \
        y, y1, y2,y_mlm1, \
        y1_mlm1,y2_mlm1,\
        y_mlm2,y1_mlm2,y2_mlm2,\
        num =[], [],[],[],[],[],[],[],[],[],[],[],[],list(),list(),list(),list(), 0
        for k in range(len(self.base)):
            ...
            if k in self.extract:
                ...
                if num%2==0 :
                    ...
                    (t_mlm1,t1_mlm1, t2_mlm1) = self.feat_mlm_1[num](x,edge)
                    (t_mlm2, t1_mlm2, t2_mlm2) = self.feat_mlm_2[num](x, edge)
                else:
                    (t,t1,t2)=self.feat[num](x)
                    (t_mlm1, t1_mlm1, t2_mlm1) = self.feat_mlm_1[num](x)
                    (t_mlm2, t1_mlm2, t2_mlm2) = self.feat_mlm_2[num](x)
                ...
                y_mlm1.append(t_mlm1)
                y1_mlm1.append(t1_mlm1)

                y2_mlm1.append(t2_mlm1)

                y_mlm2.append(t_mlm2)
                y1_mlm2.append(t1_mlm2)

                y2_mlm2.append(t2_mlm2)
                #y3.append(t3)

                num += 1

        tt,tt1,tt2 = self.feat[num](self.pool(x))
        tt_mlm1, tt1_mlm1, tt2_mlm1 = self.feat_mlm_1[num](self.pool(x))
        tt_mlm2, tt1_mlm2, tt2_mlm2 = self.feat_mlm_2[num](self.pool(x))

        y.append(tt)
        y1.append(tt1)
        y2.append(tt2)
        y_mlm1.append(tt_mlm1)
        y1_mlm1.append(tt1_mlm1)
        y2_mlm1.append(tt2_mlm1)
        y_mlm2.append(tt_mlm2)
        y1_mlm2.append(tt1_mlm2)
        y2_mlm2.append(tt2_mlm2)
        num = 0
        for k in range(16):
        ...
        for i in range(6):
            ...
            m_mlm1.append(y2_mlm1[i])
 
            m_mlm1[i] = nn.Sigmoid()(m_mlm1[i])

            m_mlm2.append(y2_mlm2[i])

            m_mlm2[i] = nn.Sigmoid()(m_mlm2[i])
        ...
        return (y,m,m_mlm1,m_mlm2,e,edges)
