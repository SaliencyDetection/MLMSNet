...
DE_optimizer1 =  optim.Adam(D_E.parameters(), lr=config.D_LEARNING_RATE/10,betas=(0.9,0.999))
DE_optimizer2 =  optim.Adam(D_E.parameters(), lr=config.D_LEARNING_RATE/10,betas=(0.9,0.999))
DE_optimizer3 =  optim.Adam(D_E.parameters(), lr=config.D_LEARNING_RATE/10,betas=(0.9,0.999))
...
for epoch in range(1, config.NUM_EPOCHS + 1):
    ...
    for iter_cnt,(img,img_e,sal_l,sal_e,ed_l,s_ln,w_e,w_s_e,w_s_m,labels,e_labels) in enumerate(train_data):
        ...
        if dd == True:
            ##fake
            f, m,m_1,m_2, e ,PRE_E= D_E(img_batch,img_e)
            ...
            if mlm == True:
                ...
                m_label = []
                L_mlm_1=0
                for iter_mlm,xx in enumerate(m):
                    m_label.append(xx.detach())
                    L_mlm_1 += F.binary_cross_entropy(m_1[iter_mlm], m_label[iter_mlm])
    ...
    DE_optimizer.param_groups[0]['lr'] = 0.95 * DE_optimizer.optimizers[0].param_groups[0]['lr']
    DE_optimizer1.param_groups[0]['lr']=0.95 * DE_optimizer1.optimizers[0].param_groups[0]['lr']
    DE_optimizer2.param_groups[0]['lr']=0.95 * DE_optimizer2.optimizers[0].param_groups[0]['lr']
    DE_optimizer3.param_groups[0]['lr']=0.95 * DE_optimizer3.optimizers[0].param_groups[0]['lr']
    U_optimizer.param_groups[0]['lr'] = 0.95 * U_optimizer.optimizers[1].param_groups[0]['lr']
